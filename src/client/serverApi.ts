    ...
    const api = {
        ...
        auth: {
            authenticate: (email: string, password: string) =>
                axios.post<{ success: boolean, token: string }>(`${urlBase}/auth`,
                    {
                        email,
                        password,
                    },
        ...
        project: {
            getAll: () =>
                axios.get<IProject[]>(`${urlBase}/system/project`,
                    baseConfig
                ),
            get: (projectId: number) =>
                axios.get<{ sourcecode: string }>(`${urlBase}/system/project/${projectId}/sourcecode`,
                    baseConfig
                ),
            save: (projectId: number, sourcecode: string) =>
                axios.post<{ success: boolean }>(`${urlBase}/system/project/${projectId}/sourcecode`,
                    {
                        sourcecode
                    },
                    baseConfig
            ...
            create: (name: string, sourceCode: string) =>
                axios.post<{ projectId: number, projectName: string, projectSourceCode: string }>(`${urlBase}/system/project/sourcecode`,
                    {
                        name,
                        sourceCode,
                    },
            ...
            delete: (projectId: number) =>
                axios.delete(`${urlBase}/system/project/${projectId}`,
                    baseConfig
                ),
            fileUpload: (projectData: FormData, onUploadProgress: (progressEvent: ProgressEvent) => void) =>
                axios.post<{ id: number, name: string, sourcecode: string }>(`${urlBase}/system/project/sourcecodefile`,
                    projectData,
                    {
                        ...baseConfig,
                        onUploadProgress
        ...
        register: {
            register: (email: string, firstname: string, lastname: string, password: string) =>
                axios.post<{ success: boolean }>(`${urlBase}/register`,
                    {
                        email,
                        firstname,
                        lastname,
