...
function addElementToState<T extends IAgentRecord | IPortRecord | IConnectionRecord | IPageRecord>(
    ...
    const newElementInternalId = state.lastInternalId + 1,
        ...
        stateAfterLastInternalIdUpdated = state.set('lastInternalId', newElementInternalId),
        stateAfterElementAdded = stateAfterLastInternalIdUpdated.set('alvisProject',
            fnToModifyAlvisProjectRecord(stateAfterLastInternalIdUpdated.alvisProject)(elementToAdd));
...
export default handleActions<IProjectRecord, string | [IAlvisProjectRecord, number] | IAgentRecord | IPortRecord | IConnectionRecord | IPageRecord>({
    ...
    [Actions.PROJECT_DELETE_AGENT]: (state: IProjectRecord, action: Action<string>) => {
        return state.set('alvisProject', apManager.deleteAgentInAlvisProject(state.alvisProject)(action.payload));
    },
    [Actions.PROJECT_MODIFY_AGENT]: (state: IProjectRecord, action: Action<IAgentRecord>) => {
        return state.set('alvisProject', apManager.modifyAgentInAlvisProject(state.alvisProject)(action.payload));
    },
    [Actions.PROJECT_ADD_PORT]: (state: IProjectRecord, action: Action<IPortRecord>) => {
        return addElementToState(state, action.payload, apManager.addPortToAlvisProject);
    },
    [Actions.PROJECT_DELETE_PORT]: (state: IProjectRecord, action: Action<string>) => {
        return state.set('alvisProject', apManager.deletePortInAlvisProject(state.alvisProject)(action.payload));
    },
    [Actions.PROJECT_MODIFY_PORT]: (state: IProjectRecord, action: Action<IPortRecord>) => {
        return state.set('alvisProject', apManager.modifyPortInAlvisProject(state.alvisProject)(action.payload));
    },
    [Actions.PROJECT_ADD_CONNECTION]: (state: IProjectRecord, action: Action<IConnectionRecord>) => {
        return addElementToState(state, action.payload, apManager.addConnectionToAlvisProject);
    },
    [Actions.PROJECT_DELETE_CONNECTION]: (state: IProjectRecord, action: Action<string>) => {
        return state.set('alvisProject', apManager.deleteConnectionInAlvisProject(state.alvisProject)(action.payload));
    },
    [Actions.PROJECT_MODIFY_CONNECTION]: (state: IProjectRecord, action: Action<IConnectionRecord>) => {
        return state.set('alvisProject', apManager.modifyConnectionInAlvisProject(state.alvisProject)(action.payload));
    },
}, initialState)