...
function createAddElementAction<
  T extends IAgentRecord | IPortRecord | IConnectionRecord | IPageRecord
>(actionType: string) {
  return createAction<T, T>(actionType, (element: T) => element);
}
...
function createModifyElementAction<
  T extends IAgentRecord | IPortRecord | IConnectionRecord | IPageRecord
>(actionType) {
  return createAction<T, T>(actionType, (element: T) => element);
}
...
const modifyPage = createModifyElementAction<IPageRecord>(
  Actions.PROJECT_MODIFY_PAGE,
);

const addAgent = createAddElementAction<IAgentRecord>(
  Actions.PROJECT_ADD_AGENT,
);
const deleteAgent = createDeleteElementAction(Actions.PROJECT_DELETE_AGENT);
const modifyAgent = createModifyElementAction<IAgentRecord>(
...
const modifyPort = createModifyElementAction<IPortRecord>(
  Actions.PROJECT_MODIFY_PORT,
);

const addConnection = createAddElementAction<IConnectionRecord>(
  Actions.PROJECT_ADD_CONNECTION,
);
const deleteConnection = createDeleteElementAction(
  Actions.PROJECT_DELETE_CONNECTION,
);
const modifyConnection = createModifyElementAction<IConnectionRecord>(
