...
const alvisProjectUpload = multer({ storage: alvisProjectStorage }).single(
  'alvisProjectFile',
);
...
export class ProjectRoute extends BaseRoute {
  ...
  private async postProjectSourceCode(
    req: Request,
    res: Response,
    next: NextFunction,
  ): Promise<void> {
  ...
  private async getProjectSourceCode(
    req: Request,
    res: Response,
    next: NextFunction,
  ): Promise<void> {
    try {
      ...
      fs.readFile(filePath, 'utf-8', (e, data: string) => {
        if (e) {
          throw e;
        } // TO DO: is it OK? --- NO, server will crash if file does not exists
  ...
  private async postCreateProjectFromFile(
    req: Request,
    res: Response,
    next: NextFunction,
  ): Promise<void> {
  ...
  private async postCreateProject(
    req: Request,
    res: Response,
    next: NextFunction,
  ): Promise<void> {
        ...
        async (e: NodeJS.ErrnoException) => {
          if (e) {
            if (e.errno === os.constants.errno.ENOENT) {
              res
                .status(404)
                .send({ error: 'Project file does not exists. :O' });
              return;
            }
  ...
  private async getProjectFilePath(projectId: number): Promise<string> {
    return new Promise<string>(async (resolve, reject) => {
      try {
        ...
        const projectFileRelativePath =
          alvisProjectsFilesDir + projectFile.realtive_path;
